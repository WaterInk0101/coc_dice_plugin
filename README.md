...
> [!NOTE]
> 该插件仍属于早期测试阶段；如游玩过程中发现不妥或bug，欢迎Issues反馈
...
2.使用 `/创建角色` 或 `/st` 命令创建属于你和伙伴们各自的角色
...
```bash
  ...
  /创建角色                                //使用默认公式3d6*5随机生成的属性进行游戏  
  
  /st [属性名][属性值] [属性名][属性值]     
  /导入 [属性名][属性值] [属性名][属性值]   //自定义录入你的属性，可同时录入多个属性，范围1~200
