...
```
 ...
 _为了方便已有角色卡导入，使用 ```/导入``` 或 ```/st``` 导入时属性间可以无需空格,例如```/st 力量21敏捷30教育2d4*5```_
...
|属性|公式|
|---|---|
|伤害加值(DB)|力量 (STR) + 体型 (SIZ)的值2-64时为-2，65-84时为-1，85-124时为0，125-164时为+1D4，165-20时为+1D6，大于等于205时为2D6|
|闪避值(DODGE)|敏捷 (DEX) ÷ 2|
