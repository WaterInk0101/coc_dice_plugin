...
def parse_import_attr_params(params: str) -> Dict[str, int]:
    ...
    for pair in attr_pairs:
        ...
        if not value_str.isdigit():
            raise ValueError(f"属性值非法：{attr_name}{value_str}（必须是1-200的整数）")

        attr_value = int(value_str)
        if attr_value < 1 or attr_value > 200:
            raise ValueError(f"属性值超出范围：{attr_name}{attr_value}（1-200）")
